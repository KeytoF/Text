...
// DialogManager.loadStyle();在header.tpl模板中已显示引入dialog.css样式文件
...
//		showloading('none');
	...
	// $$(formid).action = action.replace(/[\&|\?]inajax\=1/g, '')+(action.indexOf("?")<0?'?':'&')+'inajax=1';
    $$(formid).action = (action.indexOf('inajax=1') !== -1)?action:action+(action.indexOf("?")<0?'?':'&')+'inajax=1';
	$$(formid).submit();
	if(submitbtn) {
